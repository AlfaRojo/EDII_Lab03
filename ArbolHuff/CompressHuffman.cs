...
namespace EDII_Lab03.ArbolHuff
{
    public class CompressHuffman
    {
        ...
        {
			Directory.CreateDirectory("~/Huffman/Cifrado/");
			Directory.CreateDirectory("~/Huffman/Descifrado/");
			string direccion = string.Empty;
			string nombreArchivo = string.Empty;
			string extension = string.Empty;
			extension = Path.GetExtension(PostFile.Name);
			nombreArchivo = Path.GetFileNameWithoutExtension(PostFile.Name);
			var huffman = new Huffman();
			var fileVirtualPath = "";
			if (extension == ".huff")
			{
				using (FileStream archivo = new FileStream("~/Huffman/Descifrado/" + nombreArchivo + ".txt", FileMode.OpenOrCreate, FileAccess.ReadWrite))
				{
					int contador = 0;
					int contadorCarac = 0;
				...
				fileVirtualPath = @"~/Huffman/Descifrado/" + nombreArchivo + ".txt";
			}
			else
			{
				...
				using (FileStream archivo = new FileStream("~/Huffman/Cifrado/" + nombreArchivo + ".huff", FileMode.OpenOrCreate, FileAccess.ReadWrite))
				{
				...
				fileVirtualPath = @"~/Huffman/Cifrado/" + nombreArchivo + ".huff";
			}
			//Retornar archivo
